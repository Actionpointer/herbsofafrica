            ...
            <div class="col-lg-8 col-xlg-6 col-md-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <form class="form-horizontal form-material" action="{{route('admin.shipment.rates.update')}}" method="POST"> @csrf
                            <input type="hidden" name="rate_id" value="{{$rate->id}}">
                            <div class="form-group">
                                <label class="col-md-12 font-medium">Zone Title:</label>
                                <div class="col-md-12">
                                    <input type="text" name="name" value="{{$rate->name}}" placeholder="" class="form-control form-control-line" required>
                                </div>
                            </div>
                                ...
                                <div class="col-md-12">
                                    <select id="countries" class="form-control form-control-line select2 w-100" name="countries[]" multiple required>
                                        @foreach ($countries as $country)
                                            <option value="{{$country->id}}" data-iso="{{$country->iso}}" @if(in_array($country->id,$rate->countries)) selected @endif>{{$country->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                                ...
                                <div class="col-md-12">
                                    <select id="states" class="form-control form-control-line select2" name="states[]" multiple>
                                        @foreach ($states as $state)
                                            <option value="{{$state->id}}" @if(in_array($state->id,$rate->states)) selected @endif>{{$state->name}}</option>
                                        @endforeach
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <div class="col-md-12">
                                            <select id="state_mode" class="form-control form-control-line select2" name="states_mode">
                                                <option value="include" @if($rate->states_mode == 'include') selected @endif>Include Selected States</option>
                                                <option value="exclude" @if($rate->states_mode == 'exclude') selected @endif>Exclude Selected States</option>
                                            </select>
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        ...
                                        <div class="col-md-12">
                                            <select id="method" class="form-control form-control-line select2" name="method">
                                                <option value="flat-rate" @if($rate->method == 'flat-rate') selected @endif>Flat Rate</option>
                                                <option value="free-shipping" @if($rate->method == 'free-shipping') selected @endif>Free Shipping</option>
                                                <option value="local-pickup" @if($rate->method == 'local-pickup') selected @endif>Local Pickup</option>
                                            </select>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div class="col-md-4" id="price_type_section">
                                        ...
                                        <div class="col-md-12">
                                            <select name="price_type" id="price_type" class="form-control form-control-line select2">
                                                <option value="fixed-amount" @if($rate->price_type == 'fixed-amount') selected @endif>Fixed Amount</option>
                                                <option value="order-percentage" @if($rate->price_type == 'order-percentage') selected @endif>Order Percentage</option>    
                                            </select>
                                        </div>
                                    </div> 
                                    <div class="col-md-4" id="percentage_section" style="display: none">
                                        ...
                                        <div class="col-md-12">
                                            <div class="input-group align-items-baseline">
                                                <input type="text" name="percentage" value="{{$rate->percentage}}" placeholder="" class="form-control form-control-line">
                                        ...
                                        @foreach ($currencies as $currency)
                                            <div class="col-md-3">
                                                <div class="input-group align-items-baseline">
                                                    ...
                                                    <input type="number" step="0.1" name="prices[{{$currency->code}}]" value="{{array_key_exists($currency->code,$rate->prices) ? $rate->prices[$currency->code] : 0}}" id={{$currency->code}}  class="form-control form-control-line prices_value">
                                                </div>
                                            </div>
                                        @endforeach
                            ...
                            <div class="form-group" id="address_section" style="display: none">
                                ...
                                <div class="col-md-12">
                                    <input type="text" value="{{$rate->warehouse}}" name="warehouse" placeholder="" class="form-control form-control-line">
                                </div>
                            </div>
                            <div class="form-group">
                                <div class="col-sm-12">
    ...
    $(document).on('select2:select','#countries',function(e){
        ...
        $.ajax({
            ...
            success:function(data) {
                $('#states').children().not(':selected').remove()
